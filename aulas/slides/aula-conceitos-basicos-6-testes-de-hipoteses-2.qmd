...
- Finalidade
  - Testar se a média populacional de uma variável de interesse é igual a determinado valor especificado pelo pesquisador
...
- Condições:
  ...
  - Tamanho da amostra
    - Pequena (n<30) – Teste t Student
      - Dados com distribuição normal
    - Grande (n>30) – Teste Z
      - Não há restrição quanto a normalidade (TCL)
...
- Etapas:
  - Tomada de decisão
    - Compara-se o valor zα ou t α com o calculado
    - Com base no valor de p
    - p < α – Rejeita hipótese nula
...
# Arredondar (padrão de livro)
tabela_t[ , -1] <- round(tabela_t[ , -1], 3)

# Exibir tabela
knitr::kable(
  tabela_t,
  caption = "Tabela t de Student – Valores críticos",
  align = "c"
)
...
- Qui quadrado ($\chi^2$)
  - Finalidade
    - Verificar se a distribuição de frequências absolutas observadas na amostra difere da distribuição esperada
  - Condições
    - Variáveis qualitativas
      - Nominais
      - Ordinais
    - Se 2 categorias – frequência > 5
    - Se 3+ categorias 
      - menos de 20% das frequências <5
      - Todas >1
...
- Teste t para duas amostras independentes
  - Finalidade
    - Verificar a diferença entre as médias populacionais de dois grupos independentes
  - Condições
    - Amostras independentes
    - Normalidade dos dados
    - Calculo depende se amostras tem variâncias iguais
      - Maior s2 até 4x a menor s2, então s12 = s22
    - Testar com teste F
      - $𝐹=  \frac{(𝑠_1^2)}{(𝑠_2^2 )}$
...
$$
t = \frac{X_2 - X_1}{\sqrt{s^2(\frac{s_1^2}{n_1}+\frac{s_2^2}{n_2})}}
$$
...
- Teste t para duas amostras independentes
  - Teste F
    - Calcular a variância de cada grupo
      - Variância do grupo 1 $(𝑠_1^2)$
      - Variância do grupo 2 $(𝑠_2^2)$
    - Calcular o valor de F
      - $F =  \frac{𝑚𝑎𝑖𝑜𝑟  𝑠^2}{𝑚𝑒𝑛𝑜𝑟 𝑠^2 }$
      - $𝐹=  \frac{(𝑠_1^2)}{(𝑠_2^2 )}$
    - Comparar com Ftab
    - $g.l. \frac{𝑛−1 g.l. do numerador}{𝑛−1 g.ç. do denominador}$
...
# Tabela t de Student
# Teste t de duas amostras (bilateral)

# Graus de liberdade
gl <- 1:10

# Níveis de confiança (bilateral)
conf <- c(
  "90%"  = 0.95,
  "95%"  = 0.975,
  "99%"  = 0.995
...
# Criar tabela
tabela_t2 <- data.frame(
  `gl` = gl
)

for (nome in names(conf)) {
  tabela_t2[[nome]] <- qt(conf[nome], df = gl)
}

# Arredondar (padrão de livro)
tabela_t2[ , -1] <- round(tabela_t2[ , -1], 3)

# Exibir tabela
knitr::kable(
  tabela_t2,
  caption = "Tabela t de Student – Teste t de duas amostras (bilateral)",
  align = "c"
)
